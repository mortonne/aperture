...
params = structDefaults(params, 'patname', '',  'eventFilter', '',  'masks', {},  'lock', 1,  'overwrite', 0);
...
for c=1:length(chan)
  ...
  for s=1:length(subjpat)
    ...
    % load the pattern, grab this channel
    pattern = loadPat(subjpat(s).file, params);
  ...
  fprintf('ANOVA: ');
  pattern = NaN(ev.len, 1, length(time), length(freq));
