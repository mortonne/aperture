...
try
    ...
    % hack for new epoch rejection stat. Set the options for the new stat
    % to the same settings as the related max difference stat
    % rej = o.epoch_options.rejection_options;
    % rej.measure = [rej.measure rej.measure(3)];
    % rej.z = [rej.z rej.z(3)];
    % o.epoch_options.rejection_options = rej;
    % END HACK
    ...
    if do_auto_interp
        % NWM: below investigated whether switching to continuous,
        % non-overlapping data makes a difference in identifying
        % bad channels. Found only a negligable difference, except
        % in the Hurst exponent, where using the segmented data
        % seems to make more sense anyway. Leaving as in the
        % original code, using the overlapping segments to identify
        % bad channels.
        
        % identify bad channels
        % if epoch_overlap
        %   % create a continous dataset with no overlap; use this to
        %   % get variance and mean correlation
        %   EEG_cont = eeg_remove_epoch_overlap(EEG);
        %   cont_properties = channel_properties(EEG_cont,eeg_chans,ref_chan);
        %   clear EEG_cont
          
        %   % use the epoched set for Hurst exponent
        %   seg_properties = channel_properties(EEG, eeg_chans, ref_chan);
        %   list_properties = [cont_properties(:,1:2) seg_properties(:,3)];
        % else
