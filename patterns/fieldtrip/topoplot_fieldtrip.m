% topoplot_fieldtrip() - plot a topographic map of a scalp data field in a 2-D circular view 
...
function [handle,Zi,grid,Xi,Yi] = topoplot_fieldtrip(Values,loc_file,varargin)
...
EMARKER2LINEWIDTH = 1;
EMARKER3CHANS = [];      % mark subset of electrode locations with small disks
EMARKER3 = 'o';          % mark subset of electrode locations with small disks
EMARKER3COLOR = 'r';     % mark subset of electrode locations with small disks
EMARKERSIZE3 = 10;      % default selected channel location marker size
EMARKER3LINEWIDTH = 1;
    ...
    switch Param
	 ...
	 case 'emarker2' 
	  ...
	  EMARKER2CHANS = abs(Value{1}); % ignore channels < 0
         ...
         case 'emarker3'
          if ~iscell(Value) | length(Value) > 5
            error('''emarker3'' argument must be a cell array {chans marker color size linewidth}')
          end
          EMARKER3CHANS = abs(Value{1}); % ignore channels < 0
          if length(Value) > 1
            EMARKER3 = Value{2};
          end
          if length(Value) > 2
            EMARKER3COLOR = Value{3};
          end
          if length(Value) > 3
            EMARKERSIZE3 = Value{4};
          end
          if length(Value) > 4
            EMARKER3LINEWIDTH = Value{5};
          end
         case 'shrink'
	  shrinkfactor = Value;
	 case 'intrad'
...
if ~isempty(EMARKER3CHANS)
    if strcmpi(STYLE,'blank')
       error('emarker3 not defined for style ''blank'' - use marking channel numbers in place of data');
    else % mark1chans and mark2chans are subsets of pltchans for markers 1 and 2
       [tmp1b mark1bchans tmp2b] = setxor(pltchans,EMARKER3CHANS);
       [tmp3b tmp4b mark3chans] = intersect(EMARKER3CHANS, ...
                                            pltchans);
       mark1chans(~ismember(mark1chans,mark3chans))
    end
end
...
if strcmp(ELECTRODES,'on')   % plot electrodes as spots
  if isempty(EMARKER2CHANS)
    ...
    hp2c = plot3(y(mark3chans),x(mark3chans),ones(size((mark3chans)))*ELECTRODE_HEIGHT,...
...
elseif strcmp(ELECTRODES,'labelpoint') 
  if isempty(EMARKER2CHANS)
    ...
    hp2 = plot3(y(mark1chans),x(mark1chans),ones(size((mark1chans)))*ELECTRODE_HEIGHT,...
        EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH);
    hp2b = plot3(y(mark2chans),x(mark2chans),ones(size((mark2chans)))*ELECTRODE_HEIGHT,...
        EMARKER2,'Color',EMARKER2COLOR,'markerfacecolor', ...
                 EMARKER2COLOR,'linewidth',EMARKER2LINEWIDTH,'markersize',EMARKERSIZE2);
    hp2c = plot3(y(mark3chans),x(mark3chans),ones(size((mark3chans)))*ELECTRODE_HEIGHT,...
        EMARKER3,'Color',EMARKER3COLOR,'markerfacecolor',EMARKER3COLOR,'linewidth',EMARKER3LINEWIDTH,'markersize',EMARKERSIZE3);
  end
  for i = 1:size(labels,1)
