% headplot_fieldtrip() - plot a spherically-splined EEG field map on a semi-realistic 
...
function [HeadAxes, ColorbarHandle] = headplot_fieldtrip(values, arg1, varargin)
...
%
    ...
    if ~strcmpi(g.plotmeshonly, 'off')
        ...
        plotelecopt.plotchans = g.plotchans;
...
function plotelec(newElect, ElectrodeNames, HeadCenter, opt);
    ...
    %zach
    if isempty(opt.plotchans)
      opt.plotchans = zeros(size(newElect,1),1);
    end
    newNames = newElect*opt.NamesDFac; % Calculate electrode label positions
    for i = 1:size(newElect,1)
      %zach's crazy hack
      if opt.plotchans(i) == 1;
        line(newElect(i,1),newElect(i,2),newElect(i,3),'marker',...
             '.','markersize',30,'color','r','linestyle','none');
      elseif opt.plotchans(i) == -1;
        line(newElect(i,1),newElect(i,2),newElect(i,3),'marker',...
             '.','markersize',30,'color','b','linestyle','none');
      else
        if newElect(i,:) ~= [0 0 0]  % plot radial lines to electrode sites
          ...
          elseif opt.labelflag == 2   % plot electrode names
            ...
            line(newElect(i,1),newElect(i,2),newElect(i,3),'marker',...
                 '.','markersize',08,'color',opt.MarkerColor,'linestyle','none');
          end
        end
      end
    end
