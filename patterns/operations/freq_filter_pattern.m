...
pat = mod_pattern(pat, @filter_pat, {freq_range filttype opt}, save_opt);
  ...
  for i = 1:n_events
    for j = 1:n_chan
      for k = 1:n_freq
        ...
        y = buttfilt(x, freq_range, samplerate, filt_type, opt.order);

        % place in new pattern, without the buffer
        new_pattern(i,j,:,k) = y(buffer_samp+1:end-buffer);
      end
    end
  end
