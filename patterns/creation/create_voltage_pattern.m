...
%   replace_eegfile - [N X 2] cell array, where each row contains two strings to 
%                     be passed into strrep, to change the eegfile field in 
%                     events ({})
%   eventFilter     - input to filterStruct which designates which events to 
%                     include in the pattern ('')
%   kthresh         - kurtosis threshold; scalar indicating the maximum allowable 
%                     kurtosis for an event before it is˜excluded (5)
%
%  Channels
...
%   relativeMS      - range of times relative to the start of each event to use 
%                     for baseline subtraction ([-200 0])
%
%  Filtering
...
%
%  Z-transforming
%   ztransform      - logical; if true, voltage will be z-transformed relative to 
%                     the baseline for that session/channel (false)
%   baseEventFilter - input to filterStruct; designates which events to include for 
%                     calculating the baseline (params.eventFilter)
...
%   baseRelativeMS  - range of times relative to the start of each event to use for 
%                     baseline subtraction (params.relativeMS)
%
%  File Management
%   lock            - if true, the pattern's file will be locked during pattern 
%                     creation, and unlock when the program finishes. Useful for 
%                     processing multiple subjects in parallel on a cluster. (false)
%   overwrite       - if true, existing pattern files will be overwritten (false)
%   updateOnly      - if true, the pattern will not be created, but a pattern object 
%                     will be created and attached to the subject object (false)
%
