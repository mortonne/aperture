function [p, h, stats] = signrank_tail(x,y,varargin)
...
%   small samples.
%
%   [P,H] = SIGNRANK(...,'tail',TAIL) performs the test against the alternative
%   hypothesis specified by TAIL:
%       'both'  -- "center is not zero (or M)" (two-tailed test)
%       'right' -- "center is greater than zero (or M)" (right-tailed test)
%       'left'  -- "center is less than zero (or M)" (left-tailed test)
...
%   $Revision: 1.3 $  $Date: 2005-06-23 14:41:48 $
...
dflts   = {alpha   ''       1};
[eid,emsg,alpha,method,tail_test] = getargs(oknames,dflts,varargin{:});
if ~isempty(eid)
...
% check the tail value
if ischar(tail_test) && (size(tail_test,1)==1)
    tail_test = find(strncmpi(tail_test,{'left','both','right'},length(tail_test))) - 2;
end
if ~isscalar(tail_test) || ~isnumeric(tail_test)
...
% check to see if is above or below zero
isPos = 0;
if (w < n*(n+1)/2-w)
  isPos = 1;
end
...
elseif isequal(method,'oldexact')
    ...
    p = statsrexact_tail(tierank,w);
    p = min(1, 2*p);   % two-sided, don't double-count the middle value
end

% pbs - added to check for oneTail and set direction
if tail_test ~= 0
  % reset the p to a onetail p
  p = p/2;
  
  % invert if direction is backwards
  if (~isPos & tail_test==1) | (isPos & tail_test==-1)
    p = 1-p;
  end
end
