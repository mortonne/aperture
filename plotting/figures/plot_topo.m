...
function plot_topo(data, varargin);
  ...
  for c=1:chans, %%%%%%%% for each data channel %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if P>0 % subsequent pages (Axes specified)
      ...
      if ~isempty(g.regions)
        %y_min = min(data(:));
        %y_max = max(data(:));
        bar_y_range = (ymax - ymin) * 1/8;
        bar_y_lim = [-bar_y_range bar_y_range];
        for index=1:size(g.regions{c},2)
          tmpreg = g.regions{c}(:,index);
          if tmpreg(1) ~= tmpreg(2)
            % tmph = fill([tmpreg(1) tmpreg(2) tmpreg(2) tmpreg(1)], ...
            %              [-100 -100 100 100], [0.9 0.9 0.9]); hold on;
            patch_x = [tmpreg(1) tmpreg(2) tmpreg(2) tmpreg(1)];
            patch_y = [bar_y_lim(1) bar_y_lim(1) bar_y_lim(2) bar_y_lim(2)];
            patch_col = [.5 .5 .5];
            tmph = patch(patch_x, patch_y, patch_col); hold on;
            set(tmph, 'edgecolor', patch_col);
            %set(tmph, 'edgecolor', [.9 .9 .9]);
...
if length(g.legend) > 1 & strcmpi(g.showleg, 'on')
  ax = axes('Units', 'Normal', 'Position', [0.1 0.05 axwidth axheight]);
  axes(ax)
  axis('off')
  hold on
  for i=1:datasets
    c_ind = mod(i - 1, length(g.colors)) + 1;
    if ~iscell(g.colors{c_ind})
      tmpcolor = {g.colors{c_ind} 'linewidth' LINEWIDTH};
    else
      tmpcolor = g.colors{c_ind};
    end
    h(i) = plot(NaN, NaN, tmpcolor{1});
  end
  l = legend(ax, h, g.legend, 'Location', [0.1 0.05 axwidth axheight]);
  set(l, 'Color', get(gcf, 'Color'))
end
